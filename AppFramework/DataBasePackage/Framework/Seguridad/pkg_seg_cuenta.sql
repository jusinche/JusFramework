CREATE OR REPLACE PACKAGE PKG_SEG_CUENTA AS
        ...
        PROCEDURE PRC_CUENTA_OBT_CANT( ec_usuario VARCHAR2, ec_clave VARCHAR2, sn_cantidad out NUMBER);
         /*Obiene la cuenta  de cuentas de un usuario */
        PROCEDURE PRC_CUENTA_OBT( ec_usuario VARCHAR2, sq_resultado out sys_refcursor);
         /*Obiene la cuenta  de cuentas de un usuario */
        PROCEDURE PRC_CUENTA_ACT( en_per_id NUMBER,   ef_cue_ultimo_acceso DATE, en_cue_estado NUMBER, en_cue_accesos NUMBER,
...
CREATE OR REPLACE PACKAGE BODY PKG_SEG_CUENTA AS
    /*Obiene la cantidad de cuentas de un usuario si es mas de uno es correcto caso controrio es incorrecto*/
    PROCEDURE PRC_CUENTA_OBT_CANT( ec_usuario VARCHAR2, ec_clave VARCHAR2, sn_cantidad out NUMBER) IS
    BEGIN
            select count(*) into sn_cantidad from tseg_cuenta where cue_login=ec_usuario and cue_clave=ec_clave;
    END PRC_CUENTA_OBT_CANT;
    /*Obiene la cuenta  de cuentas de un usuario */
    PROCEDURE PRC_CUENTA_OBT( ec_usuario VARCHAR2, sq_resultado out sys_refcursor) IS
    BEGIN
